...
public class ErpProductController {
    ...
    public CommonResult<List<ErpProductRespVO>> getProductSimpleList() {
        ...
        list.forEach(product -> product.setName(product.getName()+"-"+product.getStandard()));

        return success(convertList(list, product -> new ErpProductRespVO().setId(product.getId())
                .setName(product.getName()).setBarCode(product.getBarCode()).setStandard(product.getStandard())
    ...
    @GetMapping("/getProductCommodity")
    @Operation(summary = "获取对外销售商品", description = "只包含被开启的产品，主要用于前端的下拉选项")
    public CommonResult<List<ErpProductRespVO>> getProductCommodity() {

        List<ErpProductRespVO> list =  productService.getProductCommodity();
        list.forEach(product -> product.setName(product.getName()+"-"+product.getStandard()));
        return success(list);
    }
    ...
    @GetMapping("/getProductYuan")
    @Operation(summary = "获取所有原材", description = "只包含被开启的产品，主要用于前端的下拉选项")
    public CommonResult<List<ErpProductRespVO>> getProductYuan() {

        List<ErpProductRespVO> list =  productService.getProductYuan();
        list.forEach(product -> product.setName(product.getName()+"-"+product.getStandard()));
        return success(list);
    }
